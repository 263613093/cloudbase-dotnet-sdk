...
namespace CloudBase
{
    public class Auth
    {
        ...
        private ManualResetEvent _evt = new ManualResetEvent(true);
        ...
        {
            Console.WriteLine("刷新accessToken");
            cache.RemoveStore(cache.AccessTokenKey);
            cache.RemoveStore(cache.AccessTokenExpireKey);
            string authType = cache.GetStore(cache.LoginTypeKey);
            string refreshToken = cache.GetStore(cache.RefreshTokenKey);
        ...
        {
            var accessToken = cache.GetStore(cache.AccessTokenKey);
