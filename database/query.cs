...
namespace CloudBase
{
	public partial class Query
	{
		...
		public Query OrderBy(string fieldPath, string directionStr)
		{
			...
			var newFieldOrders = new List<dynamic>();
		...
		{
			var newProjection = new Dictionary<string,int>();
			foreach (var item in projection)
			{
				newProjection[item.Key] = (item.Value ? 1 : 0);
			}
			return new Query(this.Core, this.Coll, this.FieldFilters, this.FieldOrders, this.LimitCount, this.Offset, newProjection);
		}
		private T QueryRequest<T>(string action, Dictionary<string, object> param)
		{
